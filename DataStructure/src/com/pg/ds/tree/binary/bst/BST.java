package com.pg.ds.tree.binary.bst;
...
public class BST<Key extends Comparable<Key>, Value> {
	...
	public Key ceiling(Key key) throws Exception {
		...
		if (isEmpty())
			throw new Exception("called ceiling() with empty symbol table");
	...
	public static void main(String[] args) {
		...
		byte[] byteIn = new byte[20];

		// Scanner scanInput = new Scanner(System.in);

		/*
		 * for (int i = 0; !scanInput.nextLine().isEmpty(); i++) { String key =
		 * scanInput.nextLine(); st.put(key, i); }
		 */
		int count = 0;
		while (true) {
			Scanner scanInput = new Scanner(System.in);
			String scanInStr = scanInput.nextLine();
			/*System.out.println("Entered>" + scanInStr + "<" + scanInStr.isEmpty());
			System.out.println("isEmpt? " + "".equals(scanInStr));*/
			if (scanInStr.isEmpty()) {
				//System.out.println("Empty entered");
				break;
			} else {
				if (!scanInStr.isEmpty()) {
					st.put(scanInStr, count++);
				}
			}
